...
const API_PATH = '/api/concern-reports';
...
export const ConcernInbox: React.FC = () => {
  const { data, isLoading, mutate } = useSWR<ConcernReport[]>(API_PATH);
  const items = data ?? [];
  ...
  if (items.length === 0) {
    return <p className="text-muted-foreground">Сообщений пока нет</p>;
  }
    ...
    <div className="flex flex-col gap-3">
      {items.map((item) => (
        <div
          key={item.id}
          className={cn(
            'rounded-lg border border-destructive/30 bg-card p-4 flex flex-col gap-2',
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
              <span className="flex items-center gap-1 font-medium dark:text-white">
                <User className="h-4 w-4" /> {item.reporter_name || 'Аноним'}
              </span>
              {item.contact && (
